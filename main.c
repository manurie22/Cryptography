#define CRT 0
#define SIZE 2 // 0:=256Bit 1:=512Bit 2:=1024Bit
...
void rsa_crt2();
int check_cipher(CLINT c, char* c_ext);
...
int main()
{
    ...
    SysCtlClockSet(SYSCTL_SYSDIV_4|SYSCTL_USE_PLL|SYSCTL_XTAL_16MHZ|SYSCTL_OSC_MAIN);   //Enable Clock
    ...
    // enable the GPIO pin for digital function.
        ...
        GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, GPIO_PIN_4);  //Trigger
...
*/
	...
	//check = check_cipher(c, cs_ext_S);
	check = check_cipher(c, cs_ext[SIZE]);
	...
	if (CRT) {
	    GPIOPinWrite(GPIO_PORTF_BASE,GPIO_PIN_4, 0x10); // set trigger signal
		//rsa_crt2();
		rsa_crt(c, d, p, q, m_post);
	    GPIOPinWrite(GPIO_PORTF_BASE,GPIO_PIN_4, 0x00); //reset trigger signal
	}
	else {
	    GPIOPinWrite(GPIO_PORTF_BASE,GPIO_PIN_4, 0x10); //set trigger signal
		rsa(c, d, n, m_post);
...
void rsa(CLINT base, CLINT exp, CLINT mod, CLINT res) {
    ...
    char e_bin[2000];
    char* e = e_bin;
	e = xclint2str_l(exp, 2, 0); // e_dez -> e_bin
	squ_and_mul(base, mod, res, e);
}
...
int check_cipher(CLINT c, char* cs_ext) {
    int res; 
	str2clint_l(c_ext_l, cs_ext, B10);
	if (equ_l(c,c_ext_l)) res=1;
	else res=0;
